...
class Processor:
    def __init__(self, config):
        ...
        if self.tokenizer_type == "gpt2":
            self.tokenizer = AutoTokenizer.from_pretrained(
                "gpt2", add_bos_token=False, add_eos_token=False
            )
        ...
        self.engine = InfiniGramEngine(
            index_dir=config["dir"], eos_token_id=self.tokenizer.eos_token_id
        )
    ...
    def process(self, query_type, query, query_ids, **kwargs):
        ...
        if query is not None:
            ...
            if len(query) > args.MAX_QUERY_CHARS:
                return {
                    "error": f"Please limit your input to <= {args.MAX_QUERY_CHARS} characters!"
                }
            if not (" AND " in query or " OR " in query):  # simple query
                ...
                query_ids = [
                    [self.tokenize(term) for term in terms] for terms in termss
                ]

        # validate query_ids
        if type(query_ids) == list and all(
            type(input_id) == int for input_id in query_ids
        ):  # simple query
            if len(query_ids) > args.MAX_QUERY_TOKENS:
                return {
                    "error": f"Please limit your input to <= {args.MAX_QUERY_TOKENS} tokens!"
                }
            if any(
                input_id < 0 or input_id >= self.tokenizer.vocab_size
                for input_id in query_ids
            ):
                return {"error": "Some item(s) in your query_ids are out-of-range!"}
            tokens = self.tokenizer.convert_ids_to_tokens(query_ids)
            is_cnf = False
        elif type(query_ids) == list and all(
            [
                type(clause) == list
                and all(
                    [
                        type(term) == list
                        and all([type(input_id) == int for input_id in term])
                        for term in clause
                    ]
                )
            ...
            if (
                sum(sum(len(term) for term in clause) for clause in query_ids)
            ...
            if len(query_ids) > args.MAX_CLAUSES_PER_CNF:
                return {
                    "error": f"Please enter at most {args.MAX_CLAUSES_PER_CNF} disjunctive clauses!"
                }
            for clause in query_ids:
                if len(clause) > args.MAX_TERMS_PER_CLAUSE:
                    return {
                        "error": f"Please enter at most {args.MAX_TERMS_PER_CLAUSE} terms in each disjunctive clause!"
                    }
                for term in clause:
                    if any(
                        input_id < 0 or input_id >= self.tokenizer.vocab_size
                        for input_id in term
                    ):
                        return {
                            "error": "Some item(s) in your query_ids are out-of-range!"
                        }
            tokens = [
                [self.tokenizer.convert_ids_to_tokens(term) for term in clause]
                for clause in query_ids
            ]
            is_cnf = True
        else:
            return {
                "error": "query_ids must be a list of integers, or a triply-nested list of integers!"
            }
    ...
    def count_cnf(self, query_ids, max_clause_freq=None, max_diff_tokens=None):
        if max_clause_freq is not None and not (
            type(max_clause_freq) == int
            and 0 < max_clause_freq
            and max_clause_freq <= args.MAX_CLAUSE_FREQ
        ):
            return {
                "error": f"max_clause_freq must be an integer in [1, {args.MAX_CLAUSE_FREQ}]!"
            }
        if max_diff_tokens is not None and not (
            type(max_diff_tokens) == int
            and 0 < max_diff_tokens
            and max_diff_tokens <= args.MAX_DIFF_TOKENS
        ):
            return {
                "error": f"max_diff_tokens must be an integer in [1, {args.MAX_DIFF_TOKENS}]!"
            }
        return self.engine.count_cnf(
            cnf=query_ids,
    ...
    def ntd(self, query_ids, max_support=None):
        if max_support is not None and not (
            type(max_support) == int
            and 0 < max_support
            and max_support <= args.MAX_SUPPORT
        ):
            return {
                "error": f"max_support must be an integer in [1, {args.MAX_SUPPORT}]!"
            }
        ...
        if "result_by_token_id" in result:
            for token_id in result["result_by_token_id"]:
                result["result_by_token_id"][token_id]["token"] = (
    ...
    def infgram_prob(self, query_ids):
        if len(query_ids) == 0:
            return {"error": "Please provide a non-empty query!"}
        result = self.engine.infgram_prob(
            prompt_ids=query_ids[:-1], cont_id=query_ids[-1]
        )
        if "suffix_len" in result:
            result["longest_suffix"] = self.tokenizer.decode(
                query_ids[-result["suffix_len"] - 1 : -1],
    ...
    def search_docs(self, query_ids, maxnum=None, max_disp_len=None):
        if maxnum is not None and not (
            type(maxnum) == int and 0 < maxnum and maxnum <= args.MAXNUM
        ):
            return {"error": f"maxnum must be an integer in [1, {args.MAXNUM}]!"}
        if max_disp_len is not None and not (
            type(max_disp_len) == int
            and 0 < max_disp_len
            and max_disp_len <= args.MAX_DISP_LEN
        ):
            return {
                "error": f"max_disp_len must be an integer in [1, {args.MAX_DISP_LEN}]!"
            }

        result = self.engine.search_docs(
            input_ids=query_ids, maxnum=maxnum, max_disp_len=max_disp_len
        )
        ...
        if result["cnt"] == 0:
            ...
            result["message"] = (
                f'{"Approximately " if result["approx"] else ""}{result["cnt"]} occurrences found. Displaying the documents of occurrences #{result["idxs"]}'
            )
    ...
    def search_docs_cnf(
        ...
        result = self.engine.search_docs_cnf(
            cnf=query_ids,
            maxnum=maxnum,
...
def query():
    ...
    index = (
        data["corpus"]
        if "corpus" in data
        else (data["index"] if "index" in data else None)
    )
    if "dolma-v1_6" in index and DOLMA_API_URL is not None:
        try:
            ...
            return jsonify(
                {"error": "[Flask] Web request timed out. Please try again later."}
            ), 500
    ...
    try:
        ...
        if ("query" not in data and "query_ids" not in data) or (
            ...
            return jsonify(
                {"error": "[Flask] Exactly one of query and query_ids must be present!"}
            ), 400
        if "query" in data:
            query = data["query"]
            query_ids = None
